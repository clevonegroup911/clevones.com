...
Règles applicables à tout le dépôt. Lire `PRODUCT_GOAL.md`, `backlog.json`, `PROJECT_CONTEXT.md` et `TASK_REPORT.md` avant une nouvelle exécution. X200 AUTOPLAN + FAST-LANE est la gouvernance courante. X100 reste l’historique et le contrat de compatibilité des commandes `npm run x100:*`.
...
## X200 AUTOPLAN + FAST-LANE

Objectif : planifier, exécuter, vérifier et replanifier automatiquement jusqu’à l’atteinte réelle du but produit, en supprimant les pauses administratives, répétitions et contrôles lourds inutiles sans réduire la sécurité réelle.
...
11. Si aucune tâche automatique admissible n’existe, lancer AUTOPLAN avant de conclure que le système est idle.
12. AUTOPLAN compare l’état réel à `PRODUCT_GOAL.md`, crée au maximum trois tâches utiles sans doublon, valide le registre puis rend la main à FAST-LANE.
13. Aucune tâche de remplissage n’est autorisée pour entretenir artificiellement le mouvement.
14. Le produit n’est fini que si `.x200/PRODUCT_COMPLETE.json` correspond au HEAD et au hash courants de `PRODUCT_GOAL.md` et contient des preuves concrètes.
...
- Si le backlog n’offre plus de tâche admissible, il lance AUTOPLAN pour détecter et créer le prochain travail utile à partir de `PRODUCT_GOAL.md`.
...
Aucune confirmation humaine de routine pour : lire le dépôt, analyser, planifier, créer des tâches cohérentes, modifier une branche de travail, tester, mettre à jour les rapports, pousser la branche autorisée, ouvrir ou mettre à jour une PR draft, clôturer une tâche non sensible dont les preuves sont valides, ou démarrer une autre tâche indépendante.
...
Un gate bloque seulement le travail concerné ; les autres tâches indépendantes peuvent continuer. Si tous les écarts restants nécessitent un gate humain, AUTOPLAN ne crée pas de travail fictif.
...
2. lire `PRODUCT_GOAL.md`, `backlog.json` et `TASK_REPORT.md` ;
...
7. si aucune tâche automatique n’est disponible, exécuter AUTOPLAN au lieu de s’arrêter par défaut.
...
| Autopilot + AutoPlan local | `npm run x200:autopilot:daemon` |
...
- AUTOPLAN crée au maximum trois tâches par cycle et utilise le prochain ID Txxx disponible, sans dépasser T200.
...
5. s’il n’existe plus de tâche automatique admissible, déclencher AUTOPLAN ;
6. ne pas créer une attente supplémentaire uniquement pour valider le commentaire ou le rapport qui vient d’enregistrer cette preuve.
