...
const BACKLOG_RUNTIME_ROOT_FIELDS = new Set([
  "updatedAt",
  "registryVersion",
  "nextTaskId",
  "history",
...
function stripBacklogRuntime(data) {
  ...
  for (const [key, value] of Object.entries(data)) {
    if (BACKLOG_RUNTIME_ROOT_FIELDS.has(key)) {
      continue;
    }
    if (key === "tasks" && Array.isArray(value)) {
      root.tasks = value.map((task) => {
        ...
        for (const [taskKey, taskValue] of Object.entries(task)) {
          if (!BACKLOG_RUNTIME_TASK_FIELDS.has(taskKey)) {
            stableTask[taskKey] = taskValue;
          }
        }
        return stableTask;
      });
...
function backlogChangeIsRuntimeMetadata(base, head) {
  const before = gitShowJson(base, "backlog.json");
  let after = null;
  try {
    after = JSON.parse(readFileSync("backlog.json", "utf8"));
  } catch {
    return false;
  }
  if (!before || !after) {
...
export function classifyChangedFiles({ files, base, head, forceFull = false, premerge = false } = {}) {
  if (forceFull || premerge) {
    return { mode: MODES.FULL, reason: forceFull ? "forced" : "premerge" };
  }
  if (!Array.isArray(files) || files.length === 0) {
    return { mode: MODES.FULL, reason: "unknown-or-empty-diff" };
  }
  const fullFile = files.find(requiresFull);
  if (fullFile) {
    return { mode: MODES.FULL, reason: `sensitive-control:${fullFile}` };
  }
  if (files.every(isMetadataPath)) {
    if (files.includes("backlog.json") && !backlogChangeIsRuntimeMetadata(base, head)) {
...
function awaitImportFs() {
  return { appendFileSync: (path, text) => {
    const run = spawnSync(process.execPath, ["-e", "require('fs').appendFileSync(process.argv[1], process.argv[2])", path, text], { shell: false });
    if (run.status !== 0) {
...
async function main(argv) {
  ...
  const forceFull = process.env.X200_FORCE_FULL === "1";
  const premerge = process.env.X200_PREMERGE === "1";
  const files = changedFiles(base, head);
  const result = classifyChangedFiles({ files, base, head, forceFull, premerge });
