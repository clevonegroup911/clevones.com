...
- Monitoring des échecs login/MFA (comptage `AuditLog.action` seulement) : `docs/MONITORING.md` (T012). Aucune alerte cloud n'est créée par ce dépôt.
- **P2 — `deepmerge-ts` via Prisma** : `npm audit --omit=dev` signale encore GHSA-ggr8-5vv4-36mx (`deepmerge-ts < 8`). Le paquet est présent via `@prisma/client` → `@prisma/config`, mais l'exploitation exige un graphe d'objets récursif fourni à la fusion de configuration Prisma (CLI / generate), pas une requête HTTP. Aucun correctif compatible n'existe dans Prisma 6.19.3 : `deepmerge-ts@8` est une majeure, et `npm audit fix --force` proposerait un **downgrade** cassant vers `prisma@6.12.0`. Mesure : ne pas exécuter Prisma CLI sur une config non fiable ; attendre un Prisma 6 qui pince `deepmerge-ts@8`. Next.js a été patché en 15.5.25 ; `nanoid`, `postcss` et `sharp` sont forcés en versions corrigées (3.3.18, 8.5.28, 0.35.4).
